...
- Любая задача разбивается на этапы (не более 2–4 файлов на этап или один логический блок). После каждого этапа: py_compile → тесты → workflow → Checkpoint → push → проверка origin/main. Только после успешного push — переход к следующему этапу.
...
- Итоговый отчёт — ВСЕГДА один блок ```text``` (для кнопки «Копировать» в интерфейсе Replit), строго в формате: ЗАДАНИЕ / СТАТУС / ВЕРСИЯ / ЭТАП / ИЗМЕНЕНИЯ / ФАЙЛЫ / ПРОВЕРКА / ТЕСТЫ / WORKFLOW / GIT / GITHUB / ПАМЯТЬ / ИТОГ. Без markdown/HTML-таблиц и списков сравнения — это правило действует всегда, включая диагностику и read-only проверки.
