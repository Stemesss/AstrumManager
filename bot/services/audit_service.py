...
class AuditService:
    ...
    async def get_page(
        ...
        rows = await self._audit.get_page(page, PAGE_SIZE, action_types)
        total = await self._audit.count(action_types)
        total_pages = max(1, math.ceil(total / PAGE_SIZE))
        return rows, total, total_pages

    async def search(self, query: str) -> list:
        """Поиск по нику, типу действия или дате (дд.мм.гггг)."""
        return await self._audit.search(query, per_page=20)

    async def clear(self) -> int:
        """Очищает журнал, возвращает количество удалённых записей."""
        return await self._audit.clear()
