...
Следствие для AstrumManager:
  ...
  • Администраторы (Лидер / Воин / Старейшина) — кастомный титул
...
# Базовый текст роли для кастомного титула (без эмодзи — Telegram запрещает)
_TITLE_BASE: dict[UserRole, str] = {
    UserRole.LEADER:     "Лидер",       # 5 символов
    UserRole.CLAN_CHILD: "Воин",        # 4 символа
    UserRole.ELDER:      "Старейшина",  # 10 символов
}

# Полный кастомный титул (только роль, без имени) — фоллбэк
ADMIN_TITLES: dict[UserRole, str] = {
    UserRole.LEADER:     "✪ Лидер",
    UserRole.CLAN_CHILD: "✦ Воин",
    UserRole.ELDER:      "✧ Старейшина",
}

_SEP = " | "  # ASCII | — полноширинный ｜ Telegram может отклонить
_MAX_LEN = 16


def build_admin_title(role: UserRole, game_nick: str | None = None) -> str:
    """Строит строку кастомного Telegram-титула (без эмодзи, ≤ 16 символов).

    Если game_nick указан и помещается — включает имя: «Лидер | Вадим».
    Иначе возвращает базовый титул: «✪ Лидер».

    Примеры (при game_nick="Вадим"):
      LEADER     → "Лидер | Вадим"     (13 символов ✓)
      CLAN_CHILD → "Воин | Вадим"      (12 символов ✓)
      ELDER      → "Старейшина | Вад"  (16 символов, имя усечено)
    """
    base = _TITLE_BASE.get(role)
    if not base:
        return ""  # MEMBER — без титула

    if not game_nick:
        return ADMIN_TITLES.get(role, base)
    ...
    # Имя не влезает совсем — только базовый титул с иконкой
    return ADMIN_TITLES.get(role, base)
...
async def sync_admin_title(
    ...
    • LEADER / CLAN_CHILD / ELDER → промоут + кастомный титул с именем.
    ...
    Пример кастомного титула:
      role=LEADER, game_nick="Вадим" → «Лидер | Вадим» (видно рядом с именем в чате)
    """
