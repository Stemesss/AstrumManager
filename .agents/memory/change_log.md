...
### Изменено (1 файл)
- `bot/handlers/common.py`: в `handle_start` добавлена ветка `deep_update` (args == "update"):
  - `register_if_new` уже обновляет username + first_name → синхронизация происходит автоматически
  - Если нет game_nick → сообщение об обновлении + переход в NickSetup
  - Если ник есть → «✅ Добро пожаловать! AstrumManager успешно обновлён» + MAIN_KEYBOARD
  - Ветка расположена после is_new (новые всегда в мастер), перед deep_join и no-nick
